...
/* hide default header/footer cruft */
...
/* ── Category pill strip ── */
.pill-row { display: flex; gap: .5rem; flex-wrap: wrap; margin-bottom: 1.4rem; }
.pill {
...
.poster-placeholder {
    ...
    height: 100%; font-size: 2.5rem; color: #2a2520;
}
...
/* ── No-scroll top-level overflow fix ── */
    ...
    for r in range(rows):
        ...
        for c in range(cols):
            ...
            with colset[c]:
                ...
                # title + year
                yr_html = f"<span style='color:#c9a96e;font-size:.63rem'>{year}</span>" if year else ""
                st.markdown(
                    f"<div style='margin:.3rem 0 .4rem;"
                    f"font-size:.73rem;color:#c8bfb0;line-height:1.25;"
...
with st.sidebar:
    ...
    for key, label in CATEGORIES.items():
        ...
        style = "color:#c9a96e!important;border-color:#c9a96e!important" if is_active else ""
...
# ═════════════════════════════════════════════
    ...
    with right:
        ...
        meta_parts = []
        if year:
            meta_parts.append(f"<span class='detail-meta-item'>"
                               f"Year <span class='detail-meta-val'>{year}</span>"
                               f"</span>")
        if data.get("runtime"):
            meta_parts.append(f"<span class='detail-meta-item'>"
                               f"Runtime <span class='detail-meta-val'>"
                               f"{data['runtime']} min</span></span>")
        if data.get("original_language"):
            meta_parts.append(f"<span class='detail-meta-item'>"
                               f"Language <span class='detail-meta-val'>"
                               f"{data['original_language'].upper()}</span></span>")
        if meta_parts:
            st.markdown(
        ...
        if genres:
            ...
            st.markdown(
                f"<div style='margin-bottom:.8rem'>{tags}</div>",
                unsafe_allow_html=True,
            )
